...
def main():
    ...
    p.display.set_caption("Chess game", "None")
    isPlaying = True   # Start a game
    playerOne = True
    playerTwo = False

    loadImages()        # Load images of pieces, board
    clock = p.time.Clock()
    gameState = Rule.GameState()
    ...
    while isPlaying:
        ...
        if not gameOver and not humanTurn and motlan:
            move = Algorithm.findBestMoveMinMax(gameState, validMoves)
            if move is None:
                move = Algorithm.findRandomMove(validMoves)
...
def loadImages():
    ...
    for piece in pieces:
        IMAGES[piece] = p.transform.scale(p.image.load(
            "chessPieces/" + piece + ".png"), (SQ_SIZE, SQ_SIZE))
